...
//    Litecoin and modified for Graphene.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated Graphene network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for Graphene
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the Graphene network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Graphene is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Graphene is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Graphene is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Graphene is downloading blocks...");
